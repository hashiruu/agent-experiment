...
**你要做的只有一件事：把想跑的实验一次性写进 `docs/TODO.md`。** 剩下的它自己来——
看哪块卡是真空着的，把队列里的任务铺上去，一个跑完立刻补下一个。你睡觉、上课、开会的时候，
机器不会空转等你回来敲命令。手上卡越多，这个差距越大：四块卡就是四条链同时在跑，
而你只需要每隔几小时看一眼队列。

多卡同时跑，难的从来不是"起任务"，是**几条链一起跑而不互相踩**。你不用自己操心的部分：

- **这块卡是真的空吗？** `CUDA_VISIBLE_DEVICES` 的编号会骗人，`nvidia-smi` 报错也不代表卡坏了。
  查错一次，就是把任务压到已经占满的卡上，两个一起 OOM。（L2 第 12 / 13 条）
- **会不会跟自己撞车？** 起任务前先查队列，而不是只看进程列表——正在等待的调度脚本，
  看起来什么都没在跑，其实马上就要起同一条链。（L1 第 3 条）
- **撞了会不会毁数据？** 每个 run 独立目录、只写相对路径，两条链不会写进同一个文件，
  也就不会出现"这一行结果和其余行不同源"。（L2 第 14 条）
- **崩了要重跑多少？** 阶段化落盘，只重跑炸掉的那一步。最后一步 OOM，代价是几分钟，不是几十小时。（L2 第 15 条）
- **它会不会假装跑完了？** 完成标记只在全部成功后写，监控只认本轮新产生的标记。
  上一轮失败留下的旧标记，真的把监控骗过一次。（L1 第 2 条）

论文和仓库那头同理，你回来时看到的东西是可信的：

- 重跑换掉表格数字后，全文 grep 每一个旧值，不会出现"表格改了、讨论它的正文还引着旧数"。（L1 第 5 条）
- 每次 push 之后断言本地与远端一致——这行断言是因为真的静默失败过才加上的。（L3 第 20 条）
- 改完 LaTeX 一定编译。有一次编辑把 `figure` 插进了别的表格体内，静态检查全过，文档却是坏的。（L3 第 21 条）

一句实话：模板本身不带调度进程，**调度的是 agent**，这些规则是让它敢并行、且并行了不出静默错误的前提。
